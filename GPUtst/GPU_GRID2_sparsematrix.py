...
@cuda.jit     
def if_inst_dom(grid_all, grid_array_result, if_inst_dom_reserved):
    i, j = cuda.grid(2)
    if i < grid_all.shape[0] and j < grid_array_result.shape[0]:
        point1 = grid_all[i]
        point2 = grid_array_result[j]
        ...
        if temp == dim:
            count = 0
            for k in range(len(point2[dim:])):
                if point2[dim:][k] == -1:
                    break
                elif host_data[point2[dim:][k],0] == test[i,0]:
                    pass
                elif index == test[i,1]:
                    pass
                else:
                    count+=1
            if_inst_dom_reserved[i,j] = count
...
if __name__ == '__main__':
    ...
    indices = [i for i, value in enumerate(instance_reserved) if value == 1]
    # print(indices)
    ...
    device_grid_index_all = cuda.to_device(grid_index_all)
    device_grid_array_result = cuda.to_device(grid_array_result)

    if_inst_dom_reserved = np.zeros((test.shape[0],grid_array_result.shape[0]), dtype=np.float32)
    if_inst_dom[grid_size, block_size](device_grid_index_all, device_grid_array_result,if_inst_dom_reserved)
    print(if_inst_dom_reserved)
