...
def domcmp(grid_array_result, gridcmp_reserved, domcmp_reserved, device_instance_reserved, host_data):
    ...
    if i < device_instance_reserved.shape[0] and j < gridcmp_reserved.shape[1]:
        ...
        if point2[j] == True:
            for index in grid_array_result[j][dim:]:
                if index < 0:
                    break
                if point1[0] == host_data[index][0]:
                    continue
                temp = 0
                for d in range(dim):
                    if point1[d + 3] >= host_data[index][d + 3]:
                        temp += 1
                if temp == dim:
                    domcmp_reserved[i][index] = host_data[index][2]
...
if __name__ == '__main__':
    ...
    print(gridcmp_reserved.shape[0], gridcmp_reserved.shape[1])
